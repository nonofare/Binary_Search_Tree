...
int main() {
    ...
    auto *bst = new BST::BinSearTree<some_object *>();
    ...
    for (int i = 1; i <= MAX_ORDER; i++) {
        ...
        for (int j = 0; j < n; j++) {
            ...
            bst->push(so, some_objects_cmp_lgreater);
        }
        ...
        std::cout << bst->to_str(4, some_objects_to_str);
        ...
        for (int j = 0; j < m; j++) {
            ...
            so->field_1 = rnd_num(dre);
            so->field_2 = LETTERS[rnd_let(dre)];
            if (bst->find(so, some_objects_cmp_lgreater, some_objects_cmp_equal)) hits++;
            delete so;
        }
